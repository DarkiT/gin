...
type Manager struct {
	...
	prefix           string
	permissionLoader func(string) ([]string, error)
	roleLoader       func(string) ([]string, error)
...
func (m *Manager) loadPermissions(loginID string) ([]string, error) {
	if m.permissionLoader == nil {
		return []string{}, nil
	}

	permissions, err := m.permissionLoader(loginID)
	if err != nil {
	...
	if err := m.SetPermissions(loginID, permissions); err != nil {
		return nil, err
	}
...
func (m *Manager) GetPermissions(loginID string) ([]string, error) {
	...
	if !exists {
		return m.loadPermissions(loginID)
	}
...
func (m *Manager) loadRoles(loginID string) ([]string, error) {
	if m.roleLoader == nil {
		return []string{}, nil
	}

	roles, err := m.roleLoader(loginID)
	if err != nil {
	...
	if err := m.SetRoles(loginID, roles); err != nil {
		return nil, err
	}
	return roles, nil
}
