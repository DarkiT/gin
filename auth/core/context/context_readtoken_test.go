...
func TestReadTokenFromRequest_HeaderCookieQuery(t *testing.T) {
	...
	cfg.IsReadQuery = true // Query 必须显式开启才读取（避免 token 经 URL 泄漏到日志/Referer/历史）
	m := testManager(cfg)
...
func TestReadTokenFromRequest_IsReadCookieOff_QueryOffByDefault(t *testing.T) {
	// Query 默认关闭：Header/Cookie 都关闭时，即便 query 带了 token 也不应读取（防 URL 泄漏）。
	cfg := config.DefaultConfig()
	cfg.TokenName = "satoken"
	cfg.IsReadHeader = false
	...
	assert.Equal(t, "", ReadTokenFromRequest(rc, m))

	// 显式开启 IsReadQuery 后才能从 URL 读取。
	cfg.IsReadQuery = true
	m2 := testManager(cfg)
	assert.Equal(t, "from-query", ReadTokenFromRequest(rc, m2))
}
