...
func mockLogin(loginID interface{}) string {
	token, _ := helper.Login(loginID)
	return token
}

// mockLoginWithRole 模拟用户登录并设置角色
func mockLoginWithRole(t *testing.T, loginID interface{}, roles []string) string {
	token, _ := helper.Login(loginID)
	err := helper.SetRoles(loginID, roles)
	if err != nil {
...
func mockLoginWithPermission(t *testing.T, loginID interface{}, permissions []string) string {
	token, _ := helper.Login(loginID)
	err := helper.SetPermissions(loginID, permissions)
	if err != nil {
...
func TestCheckRole_WithValidRole(t *testing.T) {
	...
	// 创建一个具有 Admin 角色的用户
	token := mockLoginWithRole(t, "user123", []string{"Admin"})
...
func TestCheckRole_WithInvalidRole(t *testing.T) {
	...
	// 创建一个只有 User 角色的用户
	token := mockLoginWithRole(t, "user456", []string{"User"})
...
func TestCheckRole_MultipleRoles(t *testing.T) {
	...
	token := mockLoginWithRole(t, "superuser", []string{"SuperAdmin"})
...
func TestCheckPermission_WithValidPermission(t *testing.T) {
	...
	token := mockLoginWithPermission(t, "user789", []string{"user.read"})
...
func TestCheckDisable_IsDisabled(t *testing.T) {
	...
	// 封禁账号
	err := helper.Disable(loginID, 3600) // 封禁 1 小时
	if err != nil {
...
func TestChainedMiddleware_CheckRoleAndHandler(t *testing.T) {
	...
	// 测试具有 SuperAdmin 角色的用户
	token := mockLoginWithRole(t, "admin123", []string{"SuperAdmin"})
...
func TestChainedMiddleware_CheckRoleAndHandler_NoRole(t *testing.T) {
	...
	// 测试具有普通用户角色
	token := mockLoginWithRole(t, "user123", []string{"User"})
...
func TestGetHandler_WithNilHandler(t *testing.T) {
	...
	token := mockLoginWithRole(t, "testuser", []string{"Admin"})
...
func TestMiddleware_CheckRole(t *testing.T) {
	...
	token := mockLoginWithRole(t, "admin999", []string{"Admin"})
...
type serveHTTPHandler struct {
	_ struct{} `sa:"*:sa_check_login"`
}

func (h *serveHTTPHandler) ServeHTTP(*ginfw.Context) {}

type multiAnnotationHandler struct {
	_ struct{} `sa:"*:sa_check_permission=user.read|user.write;Other:sa_check_role=Admin"`
}

func (h *multiAnnotationHandler) ServeHTTP(*ginfw.Context) {}

func TestParseMethodAnnotation_StructTag(t *testing.T) {
	typeOfHandler := reflect.TypeOf(serveHTTPHandler{})
	ann := parseMethodAnnotation(typeOfHandler, "*")
	assert.True(t, ann.CheckLogin)
	assert.Empty(t, ann.CheckPermission)
}

func TestParseMethodAnnotation_MethodSpecific(t *testing.T) {
	typeOfHandler := reflect.TypeOf(multiAnnotationHandler{})
	ann := parseMethodAnnotation(typeOfHandler, "*")
	assert.ElementsMatch(t, []string{"user.read", "user.write"}, ann.CheckPermission)
	assert.False(t, ann.CheckLogin)
}
