...
import (
	...
	"github.com/darkit/gin/pkg/token/storage"
)
...
var (
	...
	storageProvider  StorageProvider = StorageProviderFunc(func() Storage { return storage.NewStorage() })
...
func init() {
	providerInitOnce.Do(func() {
		configureDefaultManager(nil)
	})
}

func configureDefaultManager(cfg *Config) {
	providerMu.Lock()
...
func UseStorage(storage Storage, cfg *Config) {
	...
	UseStorageProvider(StorageProviderFunc(func() Storage {
		return storage
	}), cfg)
}
...
func Login(loginID interface{}, device ...string) (string, error) {
	return helper.Login(loginID, device...)
}
...
func Kickout(loginID interface{}, device ...string) error {
	return helper.Kickout(loginID, device...)
}
...
func CheckPermissionByToken(tokenValue string, permission string) error {
	return helper.CheckPermission(tokenValue, permission)
}
...
func HasPermission(loginID interface{}, permission string) bool {
	return helper.HasPermission(loginID, permission)
}
...
func CheckPermissionAndByToken(tokenValue string, permissions []string) error {
	return helper.CheckPermissionAnd(tokenValue, permissions)
}
...
func CheckPermissionOrByToken(tokenValue string, permissions []string) error {
	return helper.CheckPermissionOr(tokenValue, permissions)
}
...
func GetPermissionListByToken(tokenValue string) ([]string, error) {
	return helper.GetPermissionList(tokenValue)
}
...
func CheckRoleByToken(tokenValue string, role string) error {
	return helper.CheckRole(tokenValue, role)
}
...
func HasRole(loginID interface{}, role string) bool {
	return helper.HasRole(loginID, role)
}
...
func CheckRoleAndByToken(tokenValue string, roles []string) error {
	return helper.CheckRoleAnd(tokenValue, roles)
}
...
func LoginWithRefreshToken(loginID interface{}, device ...string) (*RefreshTokenInfo, error) {
	return helper.LoginWithRefreshToken(loginID, device...)
}
