...
func (p *Plugin) LoginHandler(c *gin.Context) {
	...
	// 强制业务方提供凭证校验，以免误用进入生产
	if p.ValidateCredentials == nil {
		writeErrorResponse(c, core.NewError(core.CodeNotLogin, "credential validator not configured", errors.New("missing ValidateCredentials")))
		return
	}
	if err := p.ValidateCredentials(req.Username, req.Password); err != nil {
		writeErrorResponse(c, core.NewError(core.CodeNotLogin, "invalid username or password", err))
		return
	}
...
func GetSaToken(c *gin.Context) (*core.SaTokenContext, bool) {
	satoken, exists := c.Get(saContextKey)
	if !exists {
		return nil, false
	}
