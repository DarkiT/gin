...
func (c *Config) Validate() error {
	...
	if !c.TokenStyle.IsValid() {
		return fmt.Errorf("invalid token style: %s", c.TokenStyle)
	}

	// Check JWT settings
	if c.TokenStyle == TokenStyleJWT {
		switch c.JwtAlgorithm {
		case "", "HS256", "HS384", "HS512", "RS256", "RS384", "RS512":
			// ok
		default:
			return fmt.Errorf("invalid jwt algorithm: %s", c.JwtAlgorithm)
		}

		rsAlgo := c.JwtAlgorithm == "RS256" || c.JwtAlgorithm == "RS384" || c.JwtAlgorithm == "RS512"

		if rsAlgo {
			if c.JwtPrivateKey == "" {
				return fmt.Errorf("jwt private key is required when algorithm is rsa")
			}
		} else {
			if c.JwtSecretKey == "" {
				return fmt.Errorf("jwt secret key is required when algorithm is hmac")
			}
		}
	}
	...
	if c.ActiveTimeout < NoLimit {
		return fmt.Errorf("active timeout must be >= -1, got: %d", c.ActiveTimeout)
	}

	// Check MaxLoginCount
	if c.MaxLoginCount < NoLimit {
		return fmt.Errorf("max login count must be >= -1, got: %d", c.MaxLoginCount)
	}
