...
import (
	...
	"github.com/darkit/gin/pkg/token/pool"

	"github.com/darkit/gin/pkg/token/adapter"
	"github.com/darkit/gin/pkg/token/config"
	"github.com/darkit/gin/pkg/token/listener"
	"github.com/darkit/gin/pkg/token/oauth2"
	"github.com/darkit/gin/pkg/token/security"
	"github.com/darkit/gin/pkg/token/session"
	"github.com/darkit/gin/pkg/token/token"
)
...
func NewManager(storage adapter.Storage, cfg *config.Config) *Manager {
	...
	// 确保至少有一种 Token 读取方式启用，避免业务方忘记设置导致无法取 Token
	if !cfg.IsReadHeader && !cfg.IsReadCookie && !cfg.IsReadBody {
...
func (m *Manager) Login(loginID string, device ...string) (string, error) {
	...
	_ = sess.Set(SessionKeyLoginID, loginID)
	_ = sess.Set(SessionKeyDevice, deviceType)
	_ = sess.Set(SessionKeyLoginTime, time.Now().Unix())
...
func (m *Manager) renewToken(tokenKey string) {
	...
	_ = m.storage.Expire(tokenKey, expiration)
}
