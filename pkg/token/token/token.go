...
import (
	...
	"github.com/darkit/gin/pkg/token/utils"
...
func (g *Generator) generateJWT(loginID string, device string) (string, error) {
	...
	// merge custom claims
	for k, v := range g.config.JwtCustomClaims {
		claims[k] = v
	}
	...
	signingMethod, key, err := g.getJWTSigningKey()
	if err != nil {
		return "", err
	}

	token := jwt.NewWithClaims(signingMethod, claims)
	if g.config.JwtKeyID != "" {
		token.Header["kid"] = g.config.JwtKeyID
	}
...
func (g *Generator) getJWTSigningKey() (jwt.SigningMethod, any, error) {
	alg := g.config.JwtAlgorithm
	if alg == "" {
		alg = "HS256"
	}
	...
	case "HS256", "HS384", "HS512":
		secret := g.config.JwtSecretKey
		if secret == "" {
			secret = DefaultJWTSecret
		}
		return jwt.GetSigningMethod(alg), []byte(secret), nil
	case "RS256", "RS384", "RS512":
		privKey, err := parseRSAPrivateKeyFromPEM(g.config.JwtPrivateKey)
		if err != nil {
			return nil, nil, fmt.Errorf("parse rsa private key: %w", err)
		}
		return jwt.GetSigningMethod(alg), privKey, nil
	default:
		return nil, nil, fmt.Errorf("unsupported jwt algorithm: %s", alg)
	}
}
...
func (g *Generator) ParseJWT(tokenStr string) (jwt.MapClaims, error) {
	...
	expectedAlg := g.config.JwtAlgorithm
	if expectedAlg == "" {
		expectedAlg = "HS256"
	}
	...
	token, err := jwt.Parse(tokenStr, func(token *jwt.Token) (any, error) {
		...
		if alg != expectedAlg {
			return nil, fmt.Errorf("%w: %v", ErrUnexpectedSigningMethod, alg)
		}
		...
		case "RS256", "RS384", "RS512":
			pub := g.config.JwtPublicKey
			if pub == "" {
				pub = g.config.JwtPrivateKey
			}
			key, err := jwt.ParseRSAPublicKeyFromPEM([]byte(pub))
			if err != nil {
				return nil, err
			}
			return key, nil
